...
cd ../../contracts
forge script script/AiAgentDeployer.s.sol --rpc-url $RPC_URL --private-key $PRIVATE_KEY --broadcast -v
...
cast send 0x860B6912C2d0337ef05bbC89b0C2CB6CbAEAB4A5 --value 10ether --private-key 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80 --rpc-url http://localhost:8545 --gas-limit 21000
# Copy ABIs to abis directory
mkdir -p ../abis
cp out/AiAgentTaskManager.sol/AiAgentTaskManager.json ../abis/AiAgentTaskManager.json
cp out/AiAgentServiceManager.sol/AiAgentServiceManager.json ../abis/AiAgentServiceManager.json

# Extract just the abi field from each JSON file
cd ../abis
for file in AiAgentTaskManager.json AiAgentServiceManager.json; do
    jq '.abi' "$file" > "tmp.json" && mv "tmp.json" "$file"
done
rm -rf ../abis/tmp.json
